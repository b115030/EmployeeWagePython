...
class EmployeeWageComputation:
    ...
    -------
    full_time(self)
            :returns daily wage for a full time employee
    part_time(self)
            :returns daily wage for a part time employee
    absent(self)
            :returns 0
    calculate_daily_wage(self, hours)
            :returns wage of the employee for the day
    check_attendance(self)
            :returns wage after checking attendance of the employee
    calculate_wage_till_total_hour_or_days(self)
            :returns total_wage after reaching the limiting condition
    calculate_monthly_wage(self)
            :returns monthly_wage after checking attendace
    '''
    WAGE_PER_HOUR = 20
    FULL_DAY_HOUR = 8
    HALF_DAY_HOUR = 4
